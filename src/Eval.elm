...
eval expressionSource =
    let
        ( result, _, _ ) =
            traceOrEval { trace = False, maxSteps = Nothing, tcoTarget = Nothing, callCounts = Nothing } expressionSource
    in
    result
...
evalWithMaxSteps maxSteps expressionSource =
    let
        ( result, _, _ ) =
            traceOrEval { trace = False, maxSteps = maxSteps, tcoTarget = Nothing, callCounts = Nothing } expressionSource
    in
    result
...
trace expressionSource =
    traceOrEval { trace = True, maxSteps = Nothing, tcoTarget = Nothing, callCounts = Nothing } expressionSource
