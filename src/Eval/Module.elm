        ...
        Application ((Node _ (FunctionOrValue moduleName funcName)) :: args) ->
            ...
            then
                ...
                case tryInlineFunction env moduleName funcName foldedArgs of
                    Just inlinedExpr ->
                        foldConstantSubExpressions env cfg inlinedExpr
...
Returns Just with the substituted body if the function qualifies.
-}
                        ...
                        let
                            substitution =
                                List.map2 Tuple.pair paramNames args
                                    |> Dict.fromList
                        in
                        Just (substituteVars substitution funcImpl.expression)
...
substituteVars subs ((Node range expr) as node) =
    case expr of
        ...
        CaseExpression { expression, cases } ->
            Node range
                (CaseExpression
                    { expression = substituteVars subs expression
                    , cases = List.map (\( pat, body ) -> ( pat, substituteVars subs body )) cases
                    }
                )
                                    ...
                                    (case decl of
                                        LetFunction f ->
                                            ...
                                            LetFunction
                                                { f
                                                    | declaration =
                                                        Node implRange { impl | expression = substituteVars subs impl.expression }
                                                }

                                        LetDestructuring pat val ->
                                            LetDestructuring pat (substituteVars subs val)
                                    )
                            )
                            declarations
                    , expression = substituteVars subs expression
                ...
                (RecordExpr
                    (List.map
                        (\(Node fRange ( name, val )) ->
                            Node fRange ( name, substituteVars subs val )
                        )
                        fields
                    )
                )
