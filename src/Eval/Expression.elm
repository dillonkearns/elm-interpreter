            ...
            if entry.depth >= callDepth then
                ...
                let
                    ...
                    threshold =
                        ...
                        -- (commit 4e3fa15 → 300) and that regressed `warm_import_graph_change` by
                        -- ~20% AND dropped reported lint errors from 751 to 12 because legitimate
                        -- rule-eval patterns hit the 300 threshold. We also tried a `callDepth -
                        -- entry.depth` delta heuristic, but elm-review's flat iteration legitimately
                        -- reaches thousands of identical-fingerprint calls (with delta > 800), and
                        -- the JS host stack overflows somewhere between callDepth 1000 and 1500 —
                        -- there's no safe threshold in between. See `EndToEnd.elm`'s
                        -- `mutual recursion with identical args` test (skipped on main, still
                        -- present as a known-fail on this branch).
